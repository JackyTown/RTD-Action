...
class RTD(nn.Module):
    ...
    def forward(self, locations, samples, s_e_scores):
        ...
        if self.aux_loss:
            out["aux_outputs"] = self._set_aux_loss(
                outputs_class, outputs_coord, outputs_iou
            )
    ...
    def _set_aux_loss(self, outputs_class, outputs_coord, outputs_iou):
        ...
        return [
            {"pred_logits": a, "pred_boxes": b, "pred_iou": c}
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        num_all = src_logits.shape[0] * src_logits.shape[1]
        num_positive = len(idx[0])
        num_negative = num_all - num_positive
        empty_weight = torch.ones(self.num_classes + 1)
        empty_weight[0] = 1
        empty_weight[-1] = num_positive / num_negative
        ...
        loss_ce = F.cross_entropy(
            src_logits.transpose(1, 2), target_classes, empty_weight.cuda()
        )
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        loss_bbox = F.l1_loss(
            src_boxes, box_ops.prop_se_to_cl(target_boxes), reduction="none"
        )
        ...
        loss_giou = 1 - torch.diag(
            box_ops.generalized_prop_iou(box_ops.prop_cl_to_se(src_boxes), target_boxes)
        )
    ...
    def loss_iou(self, outputs, targets, indices, num_boxes):
        ...
        for i in range(bs):
            ...
            if len(target_boxes_per_seg) == 0:
                tiou = torch.zeros((len(pred_boxes_per_seg))).to(
                    pred_boxes_per_seg.device
                )
            else:
                tiou = box_ops.generalized_prop_iou(
                    box_ops.prop_cl_to_se(pred_boxes_per_seg), target_boxes_per_seg
                )
        ...
        m_ind = (
            torch.nonzero((tgt_iou <= 0.7) & (tgt_iou > 0.3))
            .squeeze()
            .cpu()
            .detach()
            .numpy()
        ...
        iou_loss = torch.sum(iou_loss * iou_mask) / (1e-6 + torch.sum(iou_mask)).float()
...
class PostProcess(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, outputs, num_frames, base=None):
        ...
        if not self.absolute_position:
            boxes = box_ops.prop_relative_to_absolute(
                boxes, base, self.window_size, self.interval
            )
        ...
        if self.stage != 3:
            results = [
                {"scores": s, "labels": l, "boxes": b, "iou_score": i}
